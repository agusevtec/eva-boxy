...
void drawer(Screen* screen, Coor pos, Coor size, bool focused) {
    Grid rest(screen, pos, size, focused);
...
`CutRow()` cuts off a horizontal slice from the top of the remaining area and returns a new Grid representing that slice.

```cpp
// First row: 2 tiles tall
Grid row1 = rest.CutRow(2);
row1.Draw(child1);

// Second row: 3 tiles tall
Grid row2 = rest.CutRow(3);
row2.Draw(child2);

// Remaining space: automatic height
Grid row3 = rest.CutRow(0);  // or rest.CutRow()
row3.Draw(child3);
```
...
Grid col1 = rest.CutCol(4);
col1.Draw(child1);

// Second column: 6 tiles wide
Grid col2 = rest.CutCol(6);
col2.Draw(child2;

// Remaining space: automatic width
Grid col3 = rest.CutCol(0);  // or rest.CutCol()
col3.Draw(child3);
```
...
Grid col1 = rest.CutCol(4);
Grid col2 = rest.CutCol(0);  // Remaining width
...
```cpp
rest.CutCol(1).Clear();  // Clears a 1-tile wide spacer column
