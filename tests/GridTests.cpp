...
test(GridTests, SliceRow_FirstRow_SetsCorrectPosition)
{
    MockScreen screen({16, 8});
    Grid grid(Coor(0, 0), Coor(16, 8));
    
    auto row = grid.SliceRow(2);
...
test(GridTests, SliceRow_MultipleRows_AdvancesPositionCorrectly)
{
    MockScreen screen({16, 8});
    Grid grid(Coor(0, 0), Coor(16, 8));
    
    auto row1 = grid.SliceRow(2);
    auto row2 = grid.SliceRow(3);
...
test(GridTests, SliceCol_WithinRow_SetsCorrectPosition)
{
    MockScreen screen({16, 8});
    Grid grid(Coor(0, 0), Coor(16, 8));
    
    auto row = grid.SliceRow(2);
    auto col = row.SliceCol(6);
...
test(GridTests, SliceCol_MultipleCols_AdvancesPositionCorrectly)
{
    MockScreen screen({16, 8});
    Grid grid(Coor(0, 0), Coor(16, 8));
    
    auto row = grid.SliceRow(2);
    auto col1 = row.SliceCol(6);
    auto col2 = row.SliceCol(4);
...
test(GridTests, Rest_UsesRemainingSpace)
{
    MockScreen screen({16, 8});
    Grid grid(Coor(0, 0), Coor(16, 8));
...
test(GridTests, Draw_Element_DrawsAtCorrectPosition)
{
    MockScreen screen({16, 8});
    Grid grid(Coor(0, 0), Coor(16, 8));
...
// test(GridTests, Blank_ClearsArea)
// {
//     MockScreen screen({16, 8});
//     Grid grid(Coor(0, 0), Coor(16, 8));
//     screen.clear();
...
test(GridTests, SliceRow_RowHeightZero_UsesRemainingHeight)
{
    MockScreen screen({16, 8});
    Grid grid(Coor(0, 0), Coor(16, 8));
    
    auto row1 = grid.SliceRow(2);
    auto row2 = grid.SliceRow(0); // Should use remaining
    
    Coor size2 = row2.GetSize();
    assertEqual(6, size2.Y); // 8 - 2 = 6
}

test(GridTests, SliceCol_ColWidthZero_UsesRemainingWidth)
{
    MockScreen screen({16, 8});
    Grid grid(Coor(0, 0), Coor(16, 8));
    ...
    auto row = grid.SliceRow(2);
    auto col1 = row.SliceCol(6);
    auto col2 = row.SliceCol(0); // Should use remaining
    
    Coor size2 = col2.GetSize();
    assertEqual(10, size2.X); // 16 - 6 = 10
}

test(GridTests, NestedGrids_NestedSlicing_WorksCorrectly)
{
    MockScreen screen({16, 8});
    Grid grid(Coor(0, 0), Coor(16, 8));
    
    auto row = grid.SliceRow(4);
    auto col = row.SliceCol(8);
    auto nestedRow = col.SliceRow(2);
    auto nestedCol = nestedRow.SliceCol(4);
