...
// 8x8 pictogram
inline const uint8_t picto_progress_32[] PROGMEM = {
    8, 8,  // (8x8 пикселей)
...
unsigned blockFillNumber(unsigned char blockNumber)
{
...
void ProgressBar::drawer(IScreen *aScreen, Coor aPos, Coor aSize, unsigned char aIsFocused)
{
    ...
    unsigned short displayNumber = maxvalue*mValue/100;
    aScreen

    print("bottom:", self.item(0, value) )
    for i in range(1, self._width-1):
        print("middle:", self.item(i, value) )
    print("top   :", self.item(self._width-1, value) )
